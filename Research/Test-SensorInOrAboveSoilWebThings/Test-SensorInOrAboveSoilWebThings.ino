...
const char *htcTypes[] = {"TemperatureSensor", nullptr};
ThingDevice wormBin("HTC22", "HTC22 Sensor", htcTypes);
ThingProperty wormBinTemperature("temperature", "", NUMBER, "TemperatureProperty");
ThingProperty wormBinHumidity("humidity", "", NUMBER, "LevelProperty");
...
void setup() {
  ...
  wormBinTemperature.unit = "degree celsius";
  
  // Set title to "Humidity"
  wormBinHumidity.title = "Humidity";
  // Set unit for humidity to %
  wormBinHumidity.unit = "percent";
  // Set humidity as read only
  wormBinHumidity.readOnly = "true";
  // Set min and max for LevelProperty
  wormBinHumidity.minimum = 0;
  wormBinHumidity.maximum = 100;

  wormBin.addProperty(&wormBinTemperature);
  wormBin.addProperty(&wormBinHumidity);
  adapter->addDevice(&wormBin);
  adapter->begin();
...
void loop() {
  ...
  value.number = t;
  wormBinTemperature.setValue(value);
  value.number = h;
  wormBinHumidity.setValue(value);
